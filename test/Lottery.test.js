...
describe("Lottery Contract", () => {
  ...
  it("requires min amt of ether to enter", async () => {
    // Note: try and catch can be used for error handling in case of async
    //       functions.
    try {
      await lottery.methods.enter().send({
        from: accounts[0],
  ...
  it("Only mgr can call pick winner", async () => {
    try {
  ...
  it("sends money to the winner and reset player array", async () => {
    await lottery.methods.enter().send({
      from: accounts[0],
